...
class MyWidget extends Widget {
	...
	public function run() {
		...
		foreach ($SQL as $key => $value) {
			...
			$html .= '<td><div class="a-30" data-id="'.$value['id'].'"><div class="a-31" style="background-image: url('.json_decode($value['img'],true)[0].')"><div class="a-38">'.$names[$value['to']].'</div></div><div class="a-32">'.$value['desc'].'</div><div class="a-34">Осталось '.$day.' '.($day===1?'День':($day >= 8)?'Дней':'Дня').'</div><div class="a-35">Цена: от '.$value['price'].' тг.</div><div class="a-36"><a href="/search?id='.$value['id'].'"><button class="a-37" onclick="return page.showFullInfo(this,page.stop(event));" data-id="'.htmlspecialchars(json_encode($value)).'">Посмотреть тур</button></a></div></div></td>';
